    ...
    pub fn start(self) {
        thread::spawn(move || {
            for incoming in self.listener.incoming() {
                ...
                let copied_request = IOHandler::build_http_request(&buffer, &request, size);
                self.sender.send(Event::HttpEvent(copied_request));
            }
        });
    }

    fn build_http_request(buffer: &[u8], request: &Request, size: usize) -> HttpRequest {
        let body = String::from_utf8_lossy(&buffer[size..]);
        let actual_body_str:&str = body.split("\r\n").collect::<Vec<&str>>()[0];

        HttpRequest::from_request(request, String::from(actual_body_str))
    }
